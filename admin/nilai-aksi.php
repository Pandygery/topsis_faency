...
<div class="container">
  ...
  <div class="panel panel-container">
    <div class="bootstrap-table">
      <form action="nilai-proses.php?proses=simpan" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
        ...
        while ($result1=mysqli_fetch_array($query1)) {
            ...
            echo "<div class='form-group'>
                  <label>$nama_kriteria <span class='text-danger'>*</span></label>
                  <select name='$id_kriteria' class='form-control kriteria-select' required>";

                  echo "<option value='' selected disabled>-- Pilih Subkriteria --</option>";
...
function validateForm() {
    var alternatif = document.getElementById('id_alternatif').value;
    if (alternatif == '') {
        alert('Pilih alternatif terlebih dahulu!');
        return false;
    }
    
    var selects = document.querySelectorAll('.kriteria-select');
      ...
      <form action="nilai-proses.php?proses=ubah" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <select class="form-control" name="id_alternatif" required>
            ...
            $query = mysqli_query($conn,"SELECT * FROM tbl_alternatif ORDER BY id_alternatif");
            while ($result = mysqli_fetch_array($query)) { 
              if ($result['id_alternatif'] != $result3['id_alternatif']) {
                echo "<option value='$result[id_alternatif]'>$result[nama_alternatif]</option>";
              }
            }
            ?>
